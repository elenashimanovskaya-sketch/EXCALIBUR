...
```bash
# 1) MCP gpt-image-2 по cover/quad-mcp-batch.json → сохранить URL в cover/quad-mcp-result.json
# 2) Один скрипт:
...
См. также `scripts/excalibur_blog_batch_drafts.py` (live publish по умолчанию, `--draft` для черновика).
